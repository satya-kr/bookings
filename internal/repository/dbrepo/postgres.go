...
func (p *postgresDBRepo) StoreReservation(res models.Reservation) (int, error) {
...
func (p *postgresDBRepo) StoreRoomRestriction(r models.RoomRestriction) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (p *postgresDBRepo) SearchAvailabilityByDates(start, end time.Time, roomID int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	row := p.DB.QueryRowContext(ctx, query, roomID, end, start)
	err := row.Scan(&numRows)
	if err != nil {
		return false, err
	}

	//true if we have room availability
	if numRows == 0 {
		return true, nil
	}
	return false, nil
...
func (p *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	//m.App.InfoLog.Println("DATES: ", start, end)
	//query := `
	//	select
	//		r.id, r.room_name
	//	from
	...
	query := `SELECT r.id, r.room_name
				FROM rooms r
				LEFT JOIN room_restrictions rr ON r.id = rr.room_id 
										   AND $1 < rr.end_date 
										   AND $2 > rr.start_date
